...
document.querySelectorAll('.navbar-nav .nav-link').forEach((link, index) => {
  ...
  heroButtons.forEach((button, index) => {
    if (content.heroButtons[index]) button.textContent = content.heroButtons[index];
  });
  ...
  setListText('#fleet .car-card h4', content.fleetCards.map((card) => card.title));
  setListText('#fleet .car-card p', content.fleetCards.map((card) => card.text));
  setListText('#fleet .car-card .btn', content.fleetCards.map((card) => card.button));

  setText('#about .eyebrow', content.aboutEyebrow);
  setText('#about h2', content.aboutTitle);
  setText('#about > .container > .row > .col-lg-6:first-child > p:last-of-type', content.aboutText);
  ...
  aboutCardTexts.forEach((card, index) => {
    if (content.aboutCardTexts[index]) card.textContent = content.aboutCardTexts[index];
  });
...
setText('#testimonials .section-heading h2', content.testimonialsTitle);
  ...
  const contactItems = document.querySelectorAll('#contact .contact-info-item h4');
  contactItems.forEach((item, index) => {
    if (content.contactItems[index]) item.textContent = content.contactItems[index];
  });
  const contactValues = document.querySelectorAll('#contact .contact-info-item p');
  contactValues.forEach((item, index) => {
    if (content.contactValues[index]) item.textContent = content.contactValues[index];
  });
  const contactButtons = document.querySelectorAll('#contact .contact-actions .btn');
...
if (contactLinks[1]) {
  ...
  setText('.footer-section .col-lg-4:nth-of-type(2) h4', content.footerLinksHeading);
  setListText('.footer-section .footer-links a', content.footerLinks);
  setText('.footer-section .col-lg-4:nth-of-type(3) h4', content.footerFollowHeading);
  setText('.footer-section .col-lg-4:nth-of-type(3) p.mt-3', content.footerRegistration);
...
const counterObserver = new IntersectionObserver((entries, observer) => {
  entries.forEach((entry) => {
    ...
    const suffix = target === 24 ? '+' : target === 12 ? '+' : target === 250 ? '+' : target === 1000 ? '+' : '';
